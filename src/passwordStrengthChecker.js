...
export default checkPasswordStrength;function checkPasswordStrength(password, options = {}) {
    const defaults = {
        minLength: 8,
        requireUppercase: true,
        requireLowercase: true,
...
function getStrengthLabel(score) {
    const labels = ["Very Weak", "Weak", "Fair", "Good", "Strong", "Very Strong"];
    return labels[Math.min(score, labels.length - 1)] || "Unknown";
}
...
export { calculatePasswordEntropy, evaluatePasswordStrength, validatePassword };function checkPasswordStrength(password, options = {}) {
    const defaults = {
        minLength: 8,
        requireUppercase: true,
        requireLowercase: true,
...
export { validatePassword, calculateStrengthScore, getStrengthLabel };function checkPasswordStrength(password, options = {}) {
    const defaults = {
        minLength: 8,
        requireUppercase: true,
        requireLowercase: true,
...
function calculateStrengthScore(password, config) {
    ...
    if (/[^A-Za-z0-9]/.test(password) && !specialCharRegex.test(password)) {
        score += 1;
    }
