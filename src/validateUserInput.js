...
function validateUserInput(username, password) {
    ...
    if (!passwordValid) {
        return { valid: false, message: "Password must be at least 8 characters and contain uppercase, lowercase, numbers, and special characters." };
    }
    
    return { valid: true, message: "Input validation successful." };
}

export { validateUserInput, validateUsername, validatePassword };function validateEmail(email) {
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  return emailRegex.test(email);
}
