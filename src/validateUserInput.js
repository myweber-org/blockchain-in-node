...
function validateUserInput(username, password) {
    ...
    if (!usernameValid) {
        return { valid: false, message: 'Username must be 3-20 alphanumeric characters or underscores' };
    }
    if (!passwordValid) {
...
module.exports = validateUserInput;function validateUserInput(username, password) {
    ...
    if (!passwordRegex.test(password)) {
        return { isValid: false, message: 'Password must be at least 8 characters long and contain at least one letter and one number.' };
    }
    
    return { isValid: true, message: 'Input validation passed.' };
}